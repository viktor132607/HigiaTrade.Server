...
public class OrderRepository(ApplicationDbContext context) : Repository<Order>(context), IOrderRepository
{
    ...
    public async Task<bool> HasConfirmedPurchaseAsync(Guid userId, Guid productId)
    {
        return await context.Orders
            .AsNoTracking()
            .AnyAsync(order =>
    ...
    public async Task<Order> ChangeStatusAsync(Guid orderId, OrderStatus newStatus)
    {
        ...
        if (dbOrder == null) throw new AppException("Order not found").SetStatusCode(404);
    ...
    {
        Order? existingOrder = await context.Orders.Include(o => o.Items).FirstOrDefaultAsync(o => o.Id == order.Id);
        if (existingOrder == null) return null;
        context.Entry(existingOrder).CurrentValues.SetValues(order);
        PropertyInfo? modifiedOnProperty = typeof(Order).GetProperty("ModifiedOn");
        if (modifiedOnProperty != null && modifiedOnProperty.PropertyType == typeof(DateTime)) modifiedOnProperty.SetValue(existingOrder, DateTime.UtcNow);
        foreach (OrderItem item in order.Items)
        {
            ...
            if (existingItem != null)
            {
                ...
                if (itemModifiedOn != null && itemModifiedOn.PropertyType == typeof(DateTime)) itemModifiedOn.SetValue(existingItem, DateTime.UtcNow);
            }
            else if (item.Id == Guid.Empty)
            {
                item.OrderId = existingOrder.Id;
                PropertyInfo? itemModifiedOn = typeof(OrderItem).GetProperty("ModifiedOn");
                if (itemModifiedOn != null && itemModifiedOn.PropertyType == typeof(DateTime)) itemModifiedOn.SetValue(item, DateTime.UtcNow);
                context.Entry(item).State = EntityState.Added;
            }
        }
        foreach (OrderItem existingItem in existingOrder.Items.ToList())
            if (!order.Items.Any(i => i.Id == existingItem.Id)) context.Remove(existingItem);
        await context.SaveChangesAsync();
