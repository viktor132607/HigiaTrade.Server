...
{
    Task<DistributionRoutesPageResponse> GetAsync(CancellationToken cancellationToken);
    Task<DistributionRouteDto> OptimizeAsync(CreateDistributionRouteRequest request, CancellationToken cancellationToken);
...
public sealed class DistributionRouteService(
    ...
    ILogger<DistributionRouteService> logger) : IDistributionRouteService
{
    ...
    public async Task<DistributionRoutesPageResponse> GetAsync(CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new DistributionRouteServiceException(400, new { message = "Въведи име на дистрибутор." });
        }
        ...
        {
            throw new DistributionRouteServiceException(400, new { message = "Избери поне една поръчка за маршрута." });
        }

        if (requestedIds.Count > MaxOrdersPerRoute)
        {
            throw new DistributionRouteServiceException(400, new { message = $"Един маршрут може да съдържа най-много {MaxOrdersPerRoute} поръчки." });
        }
        ...
        {
            throw new DistributionRouteServiceException(409, new
            {
        ...
        {
            throw new DistributionRouteServiceException(400, new { message = "Една или повече поръчки вече не съществуват." });
        }

        if (orders.Any(order => order.Status is OrderStatus.Delivered or OrderStatus.Cancelled))
        {
            throw new DistributionRouteServiceException(400, new { message = "Доставена или отказана поръчка не може да бъде добавена към маршрут." });
        }
        ...
        for (int index = 0; index < orders.Count; index++)
        {
            ...
            {
                throw new DistributionRouteServiceException(422, new
                {
    ...
    public async Task DeleteAsync(Guid routeId, CancellationToken cancellationToken)
    {
        ...
        if (removed == 0)
        {
            throw new DistributionRouteServiceException(404, new { message = "Маршрутът не е намерен." });
        }
