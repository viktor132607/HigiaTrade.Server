...
public sealed partial class InvoiceImportService(
    ApplicationDbContext db) : IInvoiceImportService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            CatalogProduct[] preparedCatalog = catalog
                .Select(product =>
                {
            ...
            string? invoiceNumber = FindInvoiceNumber(extractedText);
            string? invoiceDate = FindInvoiceDate(extractedText);
            ...
            bool duplicateInvoice =
                ...
                await InvoiceAlreadyImportedAsync(
                    invoiceNumber,
                    cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await InvoiceAlreadyImportedAsync(
            invoiceNumber,
            cancellationToken))
        {
            throw new InvoiceImportServiceException(
        ...
        try
        {
            ...
            foreach (var product in products)
            {
                ...
                int quantity = aggregated[product.Id];
